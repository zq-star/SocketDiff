...
class SocketDiagnosticDiff:
    ...
    def compare_and_save(self, current_stats, tag, trigger_info=None):
        """对比并保存差异到JSON文件"""
        diff_stats = {}
        with self.lock:
            if self.last_diagnostics is not None:
                for key in current_stats:
                    if key not in self.last_diagnostics or current_stats[key] != self.last_diagnostics[key]:
                        diff_stats[key] = {
                            'old_value': self.last_diagnostics.get(key, 'N/A'),
                            'new_value': current_stats[key]
                        }
            
            self.last_diagnostics = current_stats.copy()
